...
use crate::{loads::{load::CalculationLoad, Load}, results::{*}, settings::{self, CalculationSettings}, structure::{Element, Node}};
use crate::loads;
...
pub fn calculate(
    ...
    let calculation_loads = &loads::utils::extract_calculation_loads(elements, nodes, loads, equation_handler);
    ...
    let map = calc_internal_forces(elements, nodes, calculation_loads, &node_results, calc_settings);

    println!("Moment force at [1, 50] {}", map[&1].moment_forces[50].value_y);

    // TODO
    node_results
}

fn calc_internal_forces(elements: &Vec<Element>,
    nodes: &HashMap<i32, Node>,
    ...
    -> HashMap<i32, InternalForceResults> {
        ...
        for element in elements {
            ...
            while x <= element_length && !last {
                let moment_force_val = internal_forces::calculate_moment_at(x, element, nodes, loads, node_results);
                let axial_force_val = internal_forces::calculate_axial_force_at(x, element, nodes, loads, node_results);
                let shear_force_val = internal_forces::calculate_shear_at(x, element, nodes, loads, node_results);
                let deflection_val = deflection::calculate_at(x, element, nodes, loads, node_results);
                let axial_deformation_val = axial_deformation::calculate_at(x, element, nodes, loads, node_results);

                moment_forces.push(InternalForcePoint { force_type: ForceType::Moment, value_x: 0.0, 
                    value_y: moment_force_val, pos_on_element: x, element_number: element.number, 
                    load_comb_number: 0 
                });
                axial_forces.push(InternalForcePoint { force_type: ForceType::Axial, value_x: 0.0, 
                    value_y: axial_force_val, pos_on_element: x, element_number: element.number, 
                    load_comb_number: 0 
                });
                shear_forces.push(InternalForcePoint { force_type: ForceType::Shear, value_x: 0.0, 
                    value_y: shear_force_val, pos_on_element: x, element_number: element.number, 
                    load_comb_number: 0 
                });
                deflections.push(InternalForcePoint { force_type: ForceType::Deflection, 
                    value_x: axial_deformation_val, value_y: deflection_val, pos_on_element: x, 
                    element_number: element.number, load_comb_number: 0 
                });
            ...
            let res = InternalForceResults { element_number: element.number, axial_forces, shear_forces, moment_forces, deflections };
