...
use vputilslib::{equation_handler::{self, EquationHandler}, geometry2d::Polygon};

use crate::profile::{PolygonProfile, Profile};
...
impl RebarCollection {
    ...
    pub fn get_single_rebars(&self, profile: &Profile, equation_handler: &EquationHandler) -> Vec<CalculationRebar> {
        let mut single_rebars: Vec<CalculationRebar> = Vec::new();        
        let offset_start = equation_handler.calculate_formula(&self.offset_start).unwrap_or(0.0);
        let offset_end = equation_handler.calculate_formula(&self.offset_end).unwrap_or(0.0);
        let mut x = 0.0;
        let mut y = 0.0;
...
/// ### Arguments
/// * `x` - The x position of the rebar from 0,0 before rotating
/// * `y` - The y position of the rebar from 0,0 before rotating
...
mod reinf_tests {
    ...
    fn test_get_single_rebars_bbox() {
        ...
        let rebar_coll1 : RebarCollection = RebarCollection {
            reinf_data, offset_start, offset_end, concrete_cover: 30.0, side,
            distribution: RebarDistribution::Even { 
                diam: 16.0, count: 4, cc_left: "30".to_string(), cc_right: "30".to_string() },
