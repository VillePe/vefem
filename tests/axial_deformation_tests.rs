...
mod axial_deformation_tests {
    ...
    use vefem::{fem::axial_deformation, loads::{self, Load}, material::Steel, settings::CalculationSettings, structure::{element::MaterialType, CalculationModel, Element, Node, Profile}};
    ...
    fn t_calculate_axial_deformation_at_pl() {
        ...
        let defl = axial_deformation::calculate_at(1000.0, &calc_model.elements[0], &calc_model.nodes, &cacl_loads, &results);
        println!("Strain(1000): {} mm", defl);
        assert_eq!(relative_eq!(defl, 0.238, epsilon = 0.01), true);
        let defl = axial_deformation::calculate_at(2000.0, &calc_model.elements[0], &calc_model.nodes, &cacl_loads, &results);
        println!("Strain(2000): {} mm", defl);
        assert_eq!(relative_eq!(defl, 0.476, epsilon = 0.01), true);        
        let defl = axial_deformation::calculate_at(3000.0, &calc_model.elements[0], &calc_model.nodes, &cacl_loads, &results);
        println!("Strain(3000): {} mm", defl);
        assert_eq!(relative_eq!(defl, 0.238, epsilon = 0.01), true);
        let defl = axial_deformation::calculate_at(4000.0, &calc_model.elements[0], &calc_model.nodes, &cacl_loads, &results);
        println!("Strain(4000): {} mm", defl);
        assert_eq!(relative_eq!(defl, 0.0, epsilon = 0.01), true);     
        
        calc_model.loads[0].rotation = -45.0;
        let cacl_loads = loads::utils::extract_calculation_loads(&calc_model.elements, &calc_model.nodes, &calc_model.loads, &EquationHandler::new());
        let results = vefem::fem::calculate(&calc_model, &mut EquationHandler::new()).node_results;
        let defl = axial_deformation::calculate_at(2000.0, &calc_model.elements[0], &calc_model.nodes, &cacl_loads, &results);  
