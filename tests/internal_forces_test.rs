...
mod internal_forces_tests {
    ...
    fn t_calculate_moment_at_pl() {
        ...
        let results = vefem::fem::calculate(
            &calc_model,
            &mut EquationHandler::new(),
        ).node_results;
        let mom = calculate_moment_at(1000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
        println!("Moment(1000): {} kNm", mom / 1e6);
        assert_eq!(relative_eq!(mom, 5e6, epsilon = 1.0), true);
        let mom = calculate_moment_at(2000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
        println!("Moment(2000): {} kNm", mom / 1e6);
        assert_eq!(relative_eq!(mom, 10e6, epsilon = 1.0), true);
        let mom = calculate_moment_at(3000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
    ...
    fn t_calculate_moment_at_rl() {
        ...
        let mom = calculate_moment_at(1999.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
    ...
    fn t_calculate_shear_at_pl() {
        ...
        let shear = calculate_shear_at(1000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
        println!("Shear(1000): {} kN", shear / 1e3);
        assert_eq!(relative_eq!(shear, 5e3, epsilon = 1.0), true);
        let shear = calculate_shear_at(2000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
        println!("Shear(2000): {} kN", shear / 1e3);
        assert_eq!(relative_eq!(shear, -5e3, epsilon = 1.0), true);
        let shear = calculate_shear_at(3000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
    ...
    fn t_calculate_shear_at_rl() {
        ...
        let shear = calculate_shear_at(1999.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
    ...
    fn t_calculate_axial_f_at_pl() {
        ...
        let axial_f = calculate_axial_force_at(1000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
        println!("Axial force(1000): {} kN", axial_f / 1e3);
        assert_eq!(relative_eq!(axial_f, 5e3, epsilon = 1.0), true);
        let axial_f = calculate_axial_force_at(2000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
        println!("Axial force(2000): {} kN", axial_f / 1e3);
        assert_eq!(relative_eq!(axial_f, -5e3, epsilon = 1.0), true);
        let axial_f = calculate_axial_force_at(3000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
    ...
    fn t_calculate_axial_f_at_rl() {
        ...
        let axial_f = calculate_axial_force_at(1999.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
        println!("Axial force(2000): {} kN", axial_f / 1e3);
        assert_eq!(relative_eq!(axial_f, 0e3, epsilon = 1.0), true);
        let axial_f = calculate_axial_force_at(2000.0, &calc_model.elements[0], &calc_model.nodes, &calc_loads, &results);
